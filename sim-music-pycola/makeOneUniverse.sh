...
sleep $[ 5 * ( $LEVEL -5 ) * $procIdx ]  # not sure if it is needed - may releave RAM congestion

echo U: PWD1= `pwd` LEVEL=$LEVEL univN=$univN
out=out_$procIdx
mkdir -p $out
...
time   ./plotCube.py --dataName  $univN  --dataPath $out --outPath $out --show bc -X -z49  >& ${out}/log.z49plot
time   ./plotCube.py --dataName  $univN  --dataPath $out --outPath $out --show bc -X -z0  >& ${out}/log.z0plot
